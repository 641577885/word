...
Page({
  ...
  getListData: function () {
    ...
    if (currentTab == 0){
      var query = { limit: 10, page: that.data.pages }
      bm.requsetData('/b/seller/chatLogList?scence=1', 'get', query, function (res) {
        console.log(res)
        if (!res.data.errcode) {
          that.setData({
        ...
        var query = { limit: 10, page: that.data.dpages }
        bm.requsetData('/b/seller/chatLogList?scence=2', 'get', query, function (res) {
        if (!res.data.errcode) {
          that.setData({
