...
Page({
  ...
  getUserInfo:function(){
    ...
    bm.requsetData('/w/user/person_info?scene=detail', 'post', '', function (data) {
      if (that.data.isfirst == 1) {
        that.setData({
          avatar: app.globalData.info.avatarUrl,
          upavatar: app.globalData.info.avatarUrl,
  ...
  onLoad: function (options) {
    var that = this;
    that.setData({ isfirst:options.isfirst})
    that.getUserInfo()
    //快速填写
