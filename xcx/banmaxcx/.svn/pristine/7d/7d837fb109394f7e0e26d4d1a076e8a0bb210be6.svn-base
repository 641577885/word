...
Page({
  ...
  Reset: function () {
    var that = this;
    that.setData({ buts: 0, fuwus1: 0, fuwus2: 0, but: 0, salary: 0, experience: 0, salaryMax: 0, salaryMin: 0 })
    console.log("已经重置！")
  },
  //确认
  Submit: function () {
  ...
  pullBottom: function () {
    ...
    if (!that.data.ispro) {//没有在请求中，开始加载数据
      if (that.data.grtdata.pages.allpage > that.data.grtdata.page) {//有下一页数据
        wx.showToast({
          mask: true,
          title: '加载中...',
          icon: 'loading',
  ...
  bm.requsetData(url, 'get', getdata, function (data) {
    ...
    if (data.data.errcode == 0) {
      ...
      setTimeout(function () {
          ...
          for (var i in that.data.grtdata.list) {
            a.push(that.data.grtdata.list[i])
          }
          that.setData({ list: a});
