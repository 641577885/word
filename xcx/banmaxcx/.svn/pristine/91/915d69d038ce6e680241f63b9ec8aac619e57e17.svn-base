...
const uploadFile = function (filePath,dir,filename, successc, failc) {
  ...
  let fileName = filename + "." + filePath.split(".").pop();//自定义图片名称

  const aliyunFileKey = path + fileName;

  const newAvatar = env.uploadImageUrl + "/" + aliyunFileKey;//最终图片cdn链接

  const aliyunServerURL = env.uploadImageUrl;//OSS存储服务器域名，需要https
