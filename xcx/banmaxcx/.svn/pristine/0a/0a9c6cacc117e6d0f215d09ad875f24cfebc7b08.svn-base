...
Page({
  ...
  ckorder:function(){
    ...
    if (that.data.currentTab == 0) {
      ...
      var query = { rtype: 2, receive_id: that.data.uid, allprice: that.data.dzprice, begintime: that.data.dzStime, endtime: that.data.dzEtime, beizhu: that.data.work_content,}
    }
    bm.requsetData('/w/resume/ckorder/create', 'post', query, function (res) {
      if (!res.data.errcode) {
        wx.showModal({
          content: '订单已生成，等待雇主确认支付',
          showCancel: false,
          confirmText: '确定',
